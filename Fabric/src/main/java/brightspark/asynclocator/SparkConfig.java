...
class SparkConfig {
    ...
    private static final Set<Class<?>> VALID_TYPES =
            Set.of(boolean.class, Boolean.class, int.class, Integer.class, float.class, Float.class, String.class);
        ...
        for (ConfigField configField : configFields) {
            ...
            if (entry != null) {
                ...
                if (!internalComment.isBlank()) {
                    ...
                    if (newValue == null && defaultValue != null) newValue = parseValueToType(defaultValue, type);
                    if (isLessThan(min, newValue))
                        throw new IllegalStateException(
                                "Config '" + name + " value '" + newValue + "' is less than the minimum of " + min);
        ...
        for (Class<?> innerClass : configClass.getDeclaredClasses()) {
            if (!Modifier.isPrivate(innerClass.getModifiers())) {
                Category category = innerClass.isAnnotationPresent(Category.class)
                        ? innerClass.getAnnotation(Category.class)
                        : null;
    ...
    private static boolean isLessThan(String min, Object value) {
        if (min == null || !(value instanceof Number)) return false;
        if (value instanceof Integer) return (Integer) value < Float.parseFloat(min);
        if (value instanceof Float) return (Float) value < Float.parseFloat(min);
        return false;
    }

    private static boolean isGreaterThan(String max, Object value) {
        if (max == null || !(value instanceof Number)) return false;
        if (value instanceof Integer) return (Integer) value > Float.parseFloat(max);
        if (value instanceof Float) return (Float) value > Float.parseFloat(max);
        return false;
    }
    ...
    private record ConfigField(Field field, Config config, ConfigCategory category) {
        ...
        StringBuilder getInternalComment() throws IllegalAccessException {
            ...
            sb.append(String.join(INTERNAL_COMMENT_SEPARATOR, internalCommentParts))
                    .append(NEW_LINE);
            return sb;
        }
