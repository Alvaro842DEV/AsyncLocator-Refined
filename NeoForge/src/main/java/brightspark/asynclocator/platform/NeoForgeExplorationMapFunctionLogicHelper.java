    ...
    public void invalidateMap(ItemStack mapStack, ServerLevel level, BlockPos invPos) {
        boolean updated = handleUpdateMapInChest(mapStack, level, invPos, (handler, slot) -> {
            ...
            if (handler instanceof CombinedResourceHandler<ItemResource> modifiableHandler) {
                ItemResourceUtils.setStackInSlot(modifiableHandler, slot, new ItemStack(Items.MAP));
            } else {
                ItemStack extracted = ItemResourceUtils.extractItem(handler, slot, mapStack.getCount(), false);
        ...
        if (!updated) {
            ALConstants.logDebug(
                    "NeoForge invalidateMap fallback: no container/slot match. Clearing pending state in-place.");
    ...
    private static boolean handleUpdateMapInChest(
        ...
        if (be != null) {
            ResourceHandler<ItemResource> itemHandler =
                    level.getCapability(Capabilities.Item.BLOCK, inventoryPos, null);
            if (itemHandler != null) {
                boolean found = false;
